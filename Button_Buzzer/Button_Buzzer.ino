...
const int BUTTON_PIN9 = 9; // Arduino pin connected to button's pin
...
const int BUTTON_PIN3 = 3; // Arduino pin connected to button's pin
const int BUTTON_PIN2 = 2; // Arduino pin connected to button's pin
const int BUZZER_PIN = 13; // Arduino pin connected to Buzzer's pin

void setup() {
  Serial.begin(9600);                // initialize serial
  pinMode(BUTTON_PIN9, INPUT_PULLUP); // set arduino pin to input pull-up mode
...
void loop() {
  int buttonState = digitalRead(BUTTON_PIN9); // read new state
  if (buttonState == LOW) {
    Serial.println("The button 9 is being pressed");
    tone(BUZZER_PIN, 100.000, 100);  // sends a frequency of 500Hz
  }

  buttonState = digitalRead(BUTTON_PIN8); // read new state
  if (buttonState == LOW) {
  ...
  buttonState = digitalRead(BUTTON_PIN3); // read new state
  if (buttonState == LOW) {
    Serial.println("The button 3 is being pressed");
    tone(BUZZER_PIN, 1396.000, 100);  // sends a frequency of 500Hz
  }

  buttonState = digitalRead(BUTTON_PIN2); // read new state
  if (buttonState == LOW) {
